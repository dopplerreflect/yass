...
<script lang="ts">
	...
	import {
		anglesArray,
		lineIntersection as lineIntersection,
		phi,
		radialPoint,
		midpoint,
		PHI,
	...
	const lineArray: Line[] = [
		...
		...angles.map(
			(a, i) => [radialPoint(a, r), radialPoint(angles[(i + 1) % angles.length], r)] as Line,
		),
		...angles.map(
			(a, i) => [radialPoint(a, r), radialPoint(angles[(i + 2) % angles.length], r)] as Line,
		),
		...angles.map(
			(a, i) => [radialPoint(a, r), radialPoint(angles[(i + 3) % angles.length], r)] as Line,
		),

		...angles.map((a, i) => [radialPoint(a + 30, r2), { x: 0, y: 0 }] as Line),
		...angles.map(
			(a, i) =>
				[radialPoint(a + 30, r2), radialPoint(angles[(i + 1) % angles.length] + 30, r2)] as Line,
		),
		...angles.map(
			(a, i) =>
				[radialPoint(a + 30, r2), radialPoint(angles[(i + 2) % angles.length] + 30, r2)] as Line,
		),
		...angles.map(
			(a, i) =>
				[radialPoint(a + 30, r2), radialPoint(angles[(i + 3) % angles.length] + 30, r2)] as Line,
		),

		...angles.map(
			(a, i) => [radialPoint(a + 30, r2), radialPoint(angles[(i + 2) % angles.length], r)] as Line,
		),
		...angles.map(
			(a, i) => [radialPoint(a + 30, r2), radialPoint(angles[(i + 3) % angles.length], r)] as Line,
		),
		...angles.map(
			(a, i) => [radialPoint(a + 30, r2), radialPoint(angles[(i + 4) % angles.length], r)] as Line,
		),
		...angles.map(
			(a, i) => [radialPoint(a + 30, r2), radialPoint(angles[(i + 5) % angles.length], r)] as Line,
		),
	];
...
<DrSvg {...{ width, height }}>
	...
	<rect
		...
		y={-height / 2}
		{...{ width, height }}
		fill={chroma.oklch(0.01, 0.0, 300).hex()}
	/>
	...
	<path
		...
		opacity={1}
		d={path}
		stroke={chroma.oklch(0.66, 0.37, 300).hex()}
