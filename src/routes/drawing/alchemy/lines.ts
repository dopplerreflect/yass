...
export function createLines(angles: number[], radii: number[]): Line[] {
	...
	const lines = [
		...
		// from outer vesica
		...angles.map((a, i) => [
			radialPoint(angles[(i + 1) % 6], radii[0], centerAtRadius0(a)),
			radialPoint(angles[(i + 3) % 6], radii[0], centerAtRadius0(angles[(i + 2) % 6]))
		] as Line),
		// 2nd hex
		...angles.map((a, i) => [
		...
		// line from 2nd hex
		...angles.map((a, i) => [
			radialPoint(angles[(i + 1) % 6], radii[1], centerAtRadius0(a)),
			radialPoint(angles[(i + 2) % 6], radii[1], centerAtRadius0(angles[(i + 3) % 6]))
		] as Line),
		// line from 3rd hex
		...angles.map((a, i) => [
